...
const T = {
  ...
  shift_morning: { de: "Frühschicht", en: "Morning shift", pl: "Zmiana poranna", ru: "Утренняя смена" },
  shift_late: { de: "Spätschicht", en: "Late shift", pl: "Zmiana wieczorna", ru: "Вечерняя смена" },
  shift_night: { de: "Nachtschicht", en: "Night shift", pl: "Zmiana nocna", ru: "Ночная смена" },
...
export default function HomePage() {
  ...
  const page: CSSProperties = {
    ...
    overflow: "hidden", // Haupt-Container bleibt ohne Scrollbar
    display: "flex",
  ...
  // Neuer, interner Scroll-Container
  const content: CSSProperties = {
    flex: 1,
    minHeight: 0,               // wichtig für flexbox-scrolling
    width: "100%",
    overflowY: "auto",          // ✅ hier darf gescrolled werden (mobil & desktop)
    WebkitOverflowScrolling: "touch" as any, // smoother iOS scroll
    textAlign: "center",
  };
  ...
  const shiftGrid: CSSProperties = {
    ...
    marginTop: 30,
    paddingBottom: 24, // etwas Luft am Ende, damit der letzte Kachel nicht am Rand klebt
  };
  ...
  /* ---------- Flaggen Dropdown (fixiert, ohne Springen) ---------- */
  ...
  const flagInner: CSSProperties = { position: "relative", width: "100%", height: "100%" };
  const flagBtn: CSSProperties = {
    width: "100%", height: "100%", borderRadius: 999, border: "1px solid #e5e7eb",
    background: "#fff", boxShadow: "0 4px 12px rgba(0,0,0,0.10)", cursor: "pointer",
    display: "grid", placeItems: "center", padding: 0,
  };
  const flagMenu: CSSProperties = {
    position: "absolute", top: 52, right: 0,
    border: "1px solid #e5e7eb", background: "#fff",
    borderRadius: 12, boxShadow: "0 8px 20px rgba(0,0,0,0.12)",
    padding: 6, display: "grid", gap: 6,
  };
  const flagItem: CSSProperties = {
    width: 44, height: 30, borderRadius: 8, overflow: "hidden",
    border: "1px solid #f3f4f6", background: "#fff", cursor: "pointer",
    padding: 0, display: "grid", placeItems: "center",
  };
  const FLAG_SRC: Record<Lang, string> = {
    de: "/flags/de.svg", en: "/flags/en.svg", pl: "/flags/pl.svg", ru: "/flags/ru.svg",
  };
  ...
  return (
    <main style={page}>
      ...
      {/* Inhaltlich scrollbarer Bereich */}
      <div style={content}>
        {/* Logo */}
